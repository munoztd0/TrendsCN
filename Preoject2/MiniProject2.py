...
def plot_sess_param(X, Y1, label1, caption=None, Y2=None,  label2=None, labelALL=None,):  
...
plot_sess_param(session, deltaBIC, '$\Delta$ BIC (model 1 - model 2)', 'Model 1 = Normal CDF+lapses, Model 2 = Repeat last-choice')
plt.savefig(homedir+"BIC.png")
...
for i in range(0,15): # loop over session data
    ...
    for k in range(1,Nmodels):
        ...
        deltaBIC.append(bic[0] - bic[k])
        if k==2:
            deltaAIC.append(aic[1] - aic[2])
            deltaBIC.append(bic[1] - bic[2])
    ...
    deltaAIC_both.append(deltaAIC[2])
    deltaBIC_both.append(deltaBIC[2])
...
plot_sess_param(session, deltaBIC_time, '$\Delta$ BIC (model 1 - model 3)', 'Model 1 = Normal CDF+lapses, Model 3 = Time-varying')
plt.savefig(homedir+"BIC_time-varying.png")

plot_sess_param(session, deltaBIC_both, '$\Delta$ BIC (model 2 - model 3)', 'Model 2 = Repeat last-choice, Model 3 = Time-varying')
plt.savefig(homedir+"BIC_both.png")
